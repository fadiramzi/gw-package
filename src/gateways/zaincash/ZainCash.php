...
namespace ninty9\superGW\gateways\zaincash;
...
class ZainCash
{
    ...
    {
      $this->_host =  Config("supergw.zain_cash.host");
        
    }
    //added by fadi
    public function passCred(array $credentials)
    {
      $credentials['msisdn'] = Env('ZC_MSISDN');
      $credentials['secret'] = Env('ZC_SECRET');
      $credentials['merchantid'] = Env('ZC_MERCHANTID');
      $credentials['production_cred'] = ( Env('ZC_ENV_PRODUCTION') === 'true' );
     ...
     */
    // charge
    public function initSession(int $amount, string $service_type, string $order_id)
    {
    ...
    private function sendHttpRequest(array $requestBody){
      $route = Config("supergw.zain_cash.endpoints.init");
        $contentType = Config("supergw.zain_cash.content_type");
        return (new Request())->request("post",$this->_host,$route,$requestBody,"",$contentType);
    }
